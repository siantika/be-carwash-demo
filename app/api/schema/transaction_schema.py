...
class ProcessTransactionRequest(BaseModel):
    ...
    ticket_id: int = Field(
        ...,
        gte=1,
        description="Ticket ID to be paid"
    )
...
class ProcessTransactionResponse(BaseModel):
    ...
    payment_method: str = Field(..., description="Payment method used (e.g. CASH, QRIS, CARD)")
    payment_metadata: Dict[str, Any] = Field(..., description="Stored payment metadata")

    subtotal_amount: Decimal = Field(..., ge=0, description="Subtotal amount before discounts")
