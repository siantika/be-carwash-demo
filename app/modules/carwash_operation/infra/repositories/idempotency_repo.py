...
class AsyncPgTicketIdempotencyRepository(IIdempotencyRepository):
        ...
        *,
        ...
        async def _fetch():
            row = await self.db.fetchrow(
                ...
                FROM carwash_operation.idempotency_keys
        ...
        return await handle_db_error(
            ...
            operation_name="fetch ticket idempotency key",
        )
        ...
        *,
        ...
        async def _create():
            try:
                row = await self.db.fetchrow(
                    f"""
                    INSERT INTO carwash_operation.idempotency_keys (
                        scope,
                        idempotency_key,
                        request_hash,
                        status,
        ...
        return await handle_db_error(
            ...
            logger=self.logger,
            context={"scope": scope, "idempotency_key": idempotency_key},
            operation_name="create ticket idempotency key",
        )
    ...
    async def mark_completed(
        ...
        *,
        ...
        async def _update():
            row = await self.db.fetchrow(
                f"""
                UPDATE carwash_operation.idempotency_keys
                SET
                    status = 'COMPLETED',
