...
class AccountResponse(BaseModel):
    id: int = Field(..., ge=1, description="Unique identifier of the account")
    username: str = Field(..., description="Username of the account")
    email: str = Field(..., description="Email address of the account")
    role: str = Field(..., description="Role assigned to the account")
...
class LoginRequest(BaseModel):
    ...
    password: str = Field(
        ..., min_length=8, max_length=64, description="Account password"
    )
